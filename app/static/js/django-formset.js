...
(function($) {
  ...
  $.fn.djangoFormset.Formset = (function() {
    function Formset(base, options) {
      ...
      this.maxForms = $("#id_" + this.prefix + "-MAX_NUM_FORMS");
      if (this.totalForms.length === 0) {
        throw new FormsetError("Management form field 'TOTAL_FORMS' not found for prefix " + this.prefix + ".");
      }
      if (this.maxForms.val() == this.totalForms.val()) {
        this.template.closest('div.formset').find(this.opts.addButtonSelector).hide();
      }
      this._initTabs();
    ...
    Formset.prototype.addForm = function() {
      ...
      if (this.maxForms.val() == this.totalForms.val()) {
        this.insertAnchor.closest('div.formset').find(this.opts.addButtonSelector).hide();
      }
      if (this.hasTabs) {
        newTab.activate();
      }
    ...
    Formset.prototype.handleFormRemoved = function(index) {
      ...
      this.totalForms.val(parseInt(this.totalForms.val()) - 1);
      if (this.maxForms.val() > this.totalForms.val()) {
        this.template.closest('div.formset').find(this.opts.addButtonSelector).show();
      }
  ...
  $.fn.djangoFormset.Form = (function() {
    ...
    Form.prototype["delete"] = function() {
      ...
      if (isInitial) {
        ...
        this.hide();
        this.formset.totalForms.val(parseInt(this.formset.totalForms.val()) - 1);
        if (this.formset.maxForms.val() > this.formset.totalForms.val()) {
          this.formset.template.closest('div.formset').find(this.formset.opts.addButtonSelector).show();
        }
      } else {
        if (this.tab) {
